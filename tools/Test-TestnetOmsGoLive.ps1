...
$parameters = @{ commands = @(
    ...
    'venue_proof=$(jq -n --slurpfile binance "${base}-binance.json" --slurpfile okx "${base}-okx.json" ''def proof($d; $uid; $allow): ($d.spec.template.spec.containers[0].env // []) as $e | {expected_uid_configured: ([$e[] | select(.name==$uid) | (((.value // "")|length)>0 or has("valueFrom"))][0] // false), allow_unverified: ([$e[] | select(.name==$allow) | .value][0] // "absent")}; {binance: proof($binance[0]; "BINANCE_VENUE_ACCOUNT_UID"; "BINANCE_ALLOW_UNVERIFIED_ACCOUNT"), okx: proof($okx[0]; "OKX_VENUE_ACCOUNT_UID"; "OKX_ALLOW_UNVERIFIED_ACCOUNT")}'' )',
