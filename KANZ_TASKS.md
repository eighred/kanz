...
**Recommended next: WIRE-01f** (cash-movement FACT producer). WIRE-01a–e have landed: events publish onto the spine; `cmd/accounting` folds live fill FACTs + live FX; `cmd/risk-engine` runs the curve-calibration scheduler; and a new `regulatory` service assembles the delivered FRTB/FormPF/AIFMD/TCFD/SFDR filings from request-supplied `*Inputs`, signed via the promoted `signer.ChainSigner` (now `internal/audit/signer`), served point-in-time + completeness-gated. Next, close the fill-only IBOR gap by emitting subscription/redemption/fee cash movements as `accounting.v1` cash FACTs the WIRE-01b folder consumes.
