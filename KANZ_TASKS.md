...
**Recommended next: WIRE-01c** (calibration Refresh scheduler). WIRE-01a (bus-producer Sink) and WIRE-01b (fill-folding consumer) have landed: events now publish onto the spine, and a running `cmd/accounting` folds live `order.v1` fill FACTs into the journal the NAV/position endpoints read (behind `ACCOUNTING_NATS_URL`; the durable Postgres journal binds when `ACCOUNTING_DATABASE_URL` is set). Next, wire the calibrator so curves refresh from the live `feed.Snapshot`.
...
_WIRE-01a/b landed: `feed.BusSink` + `SyntheticSession` publish onto the spine at `cmd/market-data` (`MARKET_DATA_FEED`); `cmd/accounting` folds live fill FACTs via `consume.NewFolder` + a `bus.Consumer` into the shared journal (`ACCOUNTING_NATS_URL`, Postgres when `ACCOUNTING_DATABASE_URL` set). The rest of the epic wires the remaining consumers of that flow._
