...
**Recommended next: WIRE-01d** (FX-rate provider for multi-currency NAV). WIRE-01a/b/c have landed: events publish onto the spine; `cmd/accounting` folds live `order.v1` fill FACTs; and `cmd/risk-engine` now runs the curve-calibration scheduler — it subscribes the market quote spine into a `livequote.LiveQuotes` cache, adapts it to `curve.QuoteSource`, and drives `curve.Calibrator.Refresh` per currency on a nightly + intraday cadence (behind `RISK_ENGINE_CALIBRATION_INTERVAL` + `_RATES`), deny-on-garbage preserved. Next, wire the accounting FX converter off a live FX `Snapshot` so NAV values a multi-currency book.
...
_WIRE-01a/b/c landed: `feed.BusSink` + `SyntheticSession` publish onto the spine at `cmd/market-data` (`MARKET_DATA_FEED`); `cmd/accounting` folds live fill FACTs via `consume.NewFolder` + a `bus.Consumer` into the shared journal (`ACCOUNTING_NATS_URL`, Postgres when `ACCOUNTING_DATABASE_URL` set); `cmd/risk-engine` runs the `schedule.Scheduler` over `curve.Calibrator`, fed by a `livequote.LiveQuotes` cache off the market spine (`RISK_ENGINE_CALIBRATION_INTERVAL` + `_RATES`). Note: the calibration loop lives in risk-engine, not market-data — the risk-module arch boundary forbids market-data importing `internal/risk/*`, and risk-engine is the boundary's designated composition root. The `schedule.Scheduler` is generic, so vol reuses it once wired; credit has no `Refresh`/`QuoteSource` seam yet (only `xva.CDSQuote`), so it is out of scope until that seam is delivered. The rest of the epic wires the remaining consumers of that flow._
