...
**Recommended next: WIRE-01b** (fold live fills into the running IBOR). WIRE-01a (the market-data bus-producer Sink — the dependency root) has landed: `feed.BusSink` publishes normalized `market.v1.MarketDataEvent` FACTs via a shared `bus.Producer`, wired at `cmd/market-data` behind `MARKET_DATA_FEED=sim` (a `SimAdapter` over a synthetic session → `Gate` → `BusSink` → spine; a real vendor `Source` plugs into the same tail). Events now flow onto the spine, so the fold/calibrate/file consumers can be wired.
...
_WIRE-01a landed: `feed.BusSink` (bus-producer Sink) + `SyntheticSession`, wired at `cmd/market-data` behind `MARKET_DATA_FEED`. Events publish onto the spine; the rest of the epic wires the consumers of that flow._

- [ ] **WIRE-01b** Wire the fill-folding consumer into the running IBOR: construct `consume.NewFolder` + a `bus.Consumer` at `cmd/accounting` subscribing `order.order.filled`/`partially_filled` (order.v1 is generated), so a running service folds live fills into the durable ledger. Reuses the risk-engine `app.NewIngest` subscription pattern; `Folder` + `ledger.FromFill` exist and are tested. · `kanz/services/accounting/cmd/accounting/`, `internal/config`
- [ ] **WIRE-01c** Calibration Refresh scheduler: an in-process scheduler driving `curve.Calibrator.Refresh` (+ vol/credit) on nightly + intraday cadence from a `Snapshot`-backed `QuoteSource`, deny-on-garbage preserved (a failed calibration leaves the prior version serving), behind an env cadence knob. Reuses the PARITY-03a `Calibrator`/`QuoteSource` seam + the `RISK_ENGINE_SNAPSHOT_INTERVAL` pattern. · `kanz/internal/risk/pricing/`, `cmd/*`
- [ ] **WIRE-01d** FX-rate provider for multi-currency NAV: an `accounting.FXConverter` backed by a live FX `Snapshot` (market-data FX quotes via WIRE-01a) + a `master`-fed instrument→currency map, so the NAV endpoint values a multi-currency book without `fx` in the request. Reuses the delivered `FXConverter`/`InstrumentCurrency` seams. Gated on WIRE-01a. · `kanz/services/accounting/`, `kanz/services/market-data/`
