...
_The credential-/infra-/human-gated half of PATH TO PARITY still waits on an external boundary landing. **The `kanz` CLI vertical is complete** (CLI-01 + CLI-02), **WIRE-03** carries the copilot citation seed end to end, and **REG-02** makes the regulatory audit chain durable across restarts. The delivered-seam backlog is now down to one surfaced correctness gap (DEBT-ARCH-01); the rest of the product surface is dependency-ordered in PATH TO PARITY. The 3-year horizon lives in `KANZ_ROADMAP.md`._
...
**Recommended next: DEBT-ARCH-01** — the one remaining buildable, non-gated in-repo gap: a delivered architecture guard (RISK-02) is red on three outsider imports of risk internals, so the boundary the arch test exists to protect is silently eroded. Closing it restores a green invariant and is pure in-repo work. Beyond it, **PS-02** (web workspace over the now-proven edge) is the next product-surface step; the rest of PATH TO PARITY is credential-/infra-/human-gated.
