...
//
// binance_ticker_test.go proves the ticker feed publishes a MarketDataEvent —
// through reconCapture, an EVENT-level double that accepts any bus.Event and
...
// services/venue-binance/internal/binance is named on its exemption list.
//
// These tests sit at the composition root because that is the layer the defect
// was at: nothing inside internal/binance could see it, and serve() dials
// Binance and a broker before it constructs anything.
...
// tickerEvent is the ticker feed's envelope, as binance_connector.go builds it:
...
func TestVenueProducerWiredFromConfigEmitsAValidatedMark(t *testing.T) {
	cfg := config.Config{Source: "venue-binance", Tenant: "acme-fund"}
	...
	if env.GetTenantId() != "acme-fund" {
		t.Errorf("TenantId = %q, want the configured VENUE_BINANCE_TENANT (acme-fund)", env.GetTenantId())
	}
}
...
func TestVenueProducerTenantIsTheServiceTenant(t *testing.T) {
	cfg := config.Config{Source: "venue-binance", Tenant: "beta-fund"}
