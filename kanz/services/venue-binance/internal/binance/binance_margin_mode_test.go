...
// had none. The behaviour was correct; nothing kept it correct.
//
...
func TestExecuteRefusesANonSpotRegimeWithoutTouchingTheExchange(t *testing.T) {
	for _, m := range []orderpb.MarginMode{
		...
		t.Run(m.String(), func(t *testing.T) {
			...
			if err == nil {
				t.Fatalf("Execute placed an order under %v — /api/v3/order is the SPOT endpoint and "+
					"carries no margin parameter, so the position would be unlevered while the audit "+
					"root recorded margin", m)
			}
			if f.posts != 0 {
				t.Fatalf("posts = %d, want 0 — the order reached Binance before being refused, which "+
					"is the whole failure: the refusal is only worth anything if nothing was placed", f.posts)
			}
			if len(fills) != 0 {
