...
import (
	...
	"github.com/eighred/kanz/services/venue-okx/internal/okx"
)
...
var balanceAnnouncementsDropped = prometheus.NewCounterVec(prometheus.CounterOpts{
	Name: "kanz_venue_okx_balance_announcements_dropped_total",
...
func serve(cfg config.Config) error {
	...
	balances := newBalanceSeams(obs.Registry, logger, "okx", cfg.Account)
	...
	if err != nil {
		return fmt.Errorf("OKX_SYMBOLS: %w", err)
	}
	...
	conn.Start(ctx, execution.WorkerDeps{
		...
		Balances: balancerecon.Announce(balanceReconConfigured, logger, "okx", balances.View),
...
func newGRPCServer(ctx context.Context, cfg config.Config, venue execution.Venue, view orderview.Store, closes execution.CloseTracker, proof execution.AccountProof, gate *halt.Gate, logger *slog.Logger) (*grpc.Server, error) {
	...
	if cfg.SPIFFESocket != "" {
		...
		ids, err := transport.ParseServiceIDs("VENUE_OKX_ALLOWED_CLIENTS", cfg.AllowedClients)
		if err != nil {
			return nil, err
		}
		...
		logger.Info("venue-okx: venue.v1 mTLS enabled", "authorized_clients", transport.ServiceIDStrings(ids))
	} else {
