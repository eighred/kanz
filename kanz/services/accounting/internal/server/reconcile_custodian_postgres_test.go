...
func newServerPool(t *testing.T) *pgxpool.Pool {
	...
	if _, err := pool.Exec(ctx,
		`DROP TABLE IF EXISTS collateral_confirmations, collateral_requests, collateral_reservations, collateral_active_agreements, collateral_workflows, collateral_snapshots, collateral_lots, custody_actions, ledger_entries, ledger_snapshots, outbox, custody_statements, custody_runs, custody_breaks CASCADE`,
	); err != nil {
		t.Fatalf("drop: %v", err)
	}
