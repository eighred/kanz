...
func TestDecodeProto_TwoAccounts(t *testing.T) {
	body, err := proto.Marshal(&wealthpb.HouseholdValued{
		...
		CurrencyCode: "USD", RecordedBy: "test:advisor", Reason: "test source",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if h.CurrencyCode != "USD" || h.AsOf.IsZero() || h.RecordedBy == "" || h.Reason == "" {
...
// Source attribution survives the projection and restart.
func TestDecodeProto_PreservesProvenanceFields(t *testing.T) {
	body, err := proto.Marshal(&wealthpb.HouseholdValued{
		HouseholdId: "HH-1",
		Accounts: []*wealthpb.ValuedAccount{
			{
...
func TestDecodeExactValuationAndMissingAmounts(t *testing.T) {
	m := &wealthpb.HouseholdValued{HouseholdId: "exact", CurrencyCode: "USD", AsOf: &timestamppb.Timestamp{Seconds: 1700000000}, RecordedBy: "test:source", Reason: "wire fixture", Accounts: []*wealthpb.ValuedAccount{{AccountId: "a", Cash: &commonpb.Decimal{Coefficient: 9007199254740993}, Holdings: []*wealthpb.ValuedHolding{{InstrumentId: "x", AssetClass: "EQUITY", MarketValue: &commonpb.Decimal{Coefficient: 1, Exponent: -18}}}}}}
	payload, err := proto.Marshal(m)
	if err != nil {
		t.Fatal(err)
	}
	h, err := DecodeProto(payload)
	if err != nil {
		t.Fatal(err)
	}
	if h.Accounts[0].Cash != "9007199254740993" || h.Accounts[0].Holdings[0].MarketValue != "0.000000000000000001" {
		t.Fatal(h)
	}
	m.Accounts[0].Cash = nil
	payload, _ = proto.Marshal(m)
	if _, err := DecodeProto(payload); err == nil {
		t.Fatal("missing cash became zero")
	}
	m.Accounts[0].Cash = &commonpb.Decimal{}
	m.Accounts[0].Holdings[0].MarketValue = nil
	payload, _ = proto.Marshal(m)
	if _, err := DecodeProto(payload); err == nil {
		t.Fatal("missing market value became zero")
	}
}
