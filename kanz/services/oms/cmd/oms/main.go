...
func runConsumers(ctx context.Context, cfg config.Config, readiness *server.Readiness, logger *slog.Logger, obs *observability.Provider) error {
	...
	// OMS-01b/c/f: order command handler over an in-memory store, the AllowAll
	// pre-trade gate (COMP-01 swaps in here), and a sim-venue router.
	emitter := order.NewEmitter(producer)
	router := execution.NewRouter(execution.NewSimVenue(cfg.SimVenueMIC))
	svc, err := order.NewService(order.NewMemoryStore(), emitter, compliance.AllowAll{}, router, logger)
	if err != nil {
		return err
	}

	// OMS-01e: fill→position projector.
	projector, err := position.NewProjector(position.NewBook(cfg.BaseCurrency), producer)
	if err != nil {
		return err
	}
