...
func TestDialVenuesRefusesAnAdapterHoldingAnotherAccount(t *testing.T) {
	...
	_, _, _, err := dialVenues(context.Background(), cfg, testVenueCounters(), quietLogger())
	if err == nil {
...
func TestDialVenuesAcceptsAnAgreeingAdapter(t *testing.T) {
	...
	venues, _, closeConns, err := dialVenues(context.Background(), cfg, testVenueCounters(), quietLogger())
	if err != nil {
...
func TestDialVenuesRefusesAnUnverifiedAccountWhenRequired(t *testing.T) {
	...
	if _, _, _, err := dialVenues(context.Background(), cfg, testVenueCounters(), quietLogger()); err == nil {
...
func TestDialVenuesBuildsTheCatalogueFromTheAdapters(t *testing.T) {
	...
	_, catalogue, closeConns, err := dialVenues(context.Background(), cfg, testVenueCounters(), quietLogger())
	if err != nil {
		t.Fatalf("dialVenues: %v", err)
	}
	t.Cleanup(closeConns)
...
func TestDialVenuesSurvivesAnAdapterThatCannotList(t *testing.T) {
	...
	venues, catalogue, closeConns, err := dialVenues(context.Background(), cfg, testVenueCounters(), quietLogger())
	if err != nil {
...
// testVenueCounters is a throwaway counter set for the cases that assert on
