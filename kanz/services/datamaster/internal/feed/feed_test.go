...
func TestSimFeed(t *testing.T) {
	f := SimFeed{
		...
		Candidates: []pricing.Candidate{
			{Source: "BLOOMBERG", Price: 100, AsOf: time.Unix(0, 0)},
		},
	}
...
func TestNormalizePrice(t *testing.T) {
	...
	a := pricing.Arbitrate("INST1", []pricing.Candidate{{Source: "BLOOMBERG", Price: 123.45, AsOf: now}}, 0, 0, now)
	...
	// 123.45 at exponent -4 ⇒ coefficient 1234500.
	if q.GetBidPrice().GetCoefficient() != 1234500 || q.GetBidPrice().GetExponent() != -4 {
		t.Errorf("bid = %+v, want 1234500e-4", q.GetBidPrice())
	}
	if q.GetBidPrice().GetCoefficient() != q.GetAskPrice().GetCoefficient() {
		t.Errorf("bid != ask for a consensus mark")
	}
	// No consensus ⇒ nil event.
	if NormalizePrice(pricing.Arbitrate("X", nil, 0, 0, now), now) != nil {
