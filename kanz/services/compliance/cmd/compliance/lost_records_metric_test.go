...
func TestBreachRecordsLostExportsZeroBeforeAnyLoss(t *testing.T) {
	reg := prometheus.NewRegistry()
	reg.MustRegister(breachRecordsLost)
	...
	for _, f := range families {
		if f.GetName() == "kanz_compliance_breach_records_lost_total" {
			got = f
		}
	}
	if got == nil {
		t.Fatal("kanz_compliance_breach_records_lost_total exports no series at startup — an operator cannot " +
