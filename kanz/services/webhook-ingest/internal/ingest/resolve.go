...
// The pipeline depends on live platform state through these narrow seams. In
...
// PositionSource returns a fund's signed net position in an instrument at a
// venue (positive long, negative short) — the OMS position projection in
// production. Needed to size a CLOSE (flatten).
...
var ErrNoAllocation = errors.New("ingest: fund has no venue allocation")
...
func (p StaticPrices) Price(_ context.Context, instrumentID string) (*big.Rat, error) {
	r, ok := p[instrumentID]
	if !ok {
		return nil, errors.New("ingest: no price for " + instrumentID)
	}
...
func (e StaticEquity) Equity(_ context.Context, fundID string) (*big.Rat, error) {
	r, ok := e[fundID]
	if !ok {
		return nil, errors.New("ingest: no equity for " + fundID)
	}
	return new(big.Rat).Set(r), nil
}
