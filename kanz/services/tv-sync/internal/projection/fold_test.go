...
func TestFold_WeightedAverageEntry(t *testing.T) {
	...
	if pos.Qty.Cmp(rat(2)) != 0 {
		t.Errorf("net = %s, want 2", pos.Qty.RatString())
	}
	if pos.AvgCost.Cmp(rat(150)) != 0 { // (100+200)/2
		t.Errorf("avg = %s, want 150", pos.AvgCost.RatString())
	}
...
func TestFold_ReduceRealizesAgainstAverage(t *testing.T) {
	...
	if pos.Qty.Cmp(rat(1)) != 0 {
		t.Errorf("net = %s, want 1", pos.Qty.RatString())
	}
	if pos.AvgCost.Cmp(rat(150)) != 0 { // average unchanged on a reduce
		t.Errorf("avg = %s, want 150", pos.AvgCost.RatString())
	}
...
func TestFold_FlipThroughZeroRebasesAverage(t *testing.T) {
	...
	if pos.Qty.Cmp(rat(-2)) != 0 {
		t.Errorf("net = %s, want -2 (flipped short)", pos.Qty.RatString())
	}
	if pos.AvgCost.Cmp(rat(200)) != 0 { // re-based at the crossing price
		t.Errorf("avg = %s, want 200", pos.AvgCost.RatString())
	}
	if pos.Realized.Cmp(rat(100)) != 0 {
		t.Errorf("realized = %s, want 100", pos.Realized.RatString())
	}
...
func TestFold_ShortThenCoverRealizes(t *testing.T) {
	...
	if pos.Qty.Sign() != 0 {
		t.Errorf("net = %s, want 0 (flat)", pos.Qty.RatString())
	}
