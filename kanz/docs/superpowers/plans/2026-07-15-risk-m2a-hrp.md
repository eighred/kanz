...
// Diversification vs min-variance: on the same two-cluster cov, min-variance
// concentrates in the low-variance {A,B} cluster (C,D have 4× the variance),
// while HRP keeps a materially larger share in the high-variance cluster. So
// w_HRP(C)+w_HRP(D) must exceed w_minvar(C)+w_minvar(D).
