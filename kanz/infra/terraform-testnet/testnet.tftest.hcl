...
run "cost_and_security_envelope" {
  ...
  assert {
    condition     = length(aws_ecr_repository.capital_path) == 14 && alltrue([for repository in aws_ecr_repository.capital_path : repository.image_tag_mutability == "IMMUTABLE"])
