...
>
> **Reconciliation note (2026-07-28, target-system pass).** A full re-scan of the repository against the target system produced `docs/ANALYSIS.md` → `docs/GAP.md` → `docs/PLAN.md`, and moved two lines on this roadmap. **EXEC-M5c is DELIVERED**, not pending — and its description was wrong in the way rule 5 exists to catch: it located future work *"behind the existing per-venue build tags"*, and those tags do not exist. **They were retired on purpose** (INFRA-M7a: exchanges became out-of-process adapters selected by `OMS_VENUE_ENDPOINTS`; the market-data feeds compile in unconditionally) — but `KANZ_BRAIN.md:17,44`, four `Makefile` targets and a CI matrix comment still describe the retired axis as live, one of them instructing operators that the shipped OMS image is a simulator that must not trade. That is tracked as BUILD-TAG-001. **The lesson is the one this file already states:** a roadmap line that points at a retired architecture does not merely waste a pass — it keeps the retired architecture alive in everyone's head.
>
> **Strategic question re-opened by the same pass, and NOT resolved here.** This roadmap records the end-user product surface (PS-01…PS-07) as **cancelled** (2026-07-13) in favour of the sovereign control plane. The target system asks for the opposite: a user-facing `kanz` terminal with fund baskets, key management and monitoring. Ground truth today: no basket entity exists in any form (`git grep -in basket` → four comments, zero schema), and **no client binary is distributable at all** — `make release` builds `./services/...` only and no release attaches a client artifact. Whether the product surface returns is a **product decision for the lead**; until it is made, BASKET-000 and TUI-TOPOLOGY-001 on the board are decisions, not engineering, and no basket implementation should start. See `docs/GAP.md` §1.1–1.2.
>
...
- **Live venue depth `DepthSource`s (EXEC-M5c). ✅ DONE** (verified 2026-07-28). The Binance and OKX L2-depth + trade websockets are wired and live: `services/market-ingest/cmd/market-ingest/sources_{binance,okx}.go` bind `alpha.BinanceFeed`/`alpha.OKXFeed` for every instrument with a mapped venue symbol, and `sources.go:14-25` records why there is no gate — *"There are no build tags any more (INFRA-M7a-4)… which ones actually run is decided at RUNTIME by the symbol maps."* **The simulator is no longer a fallback**: it used to be swapped in silently when nothing was compiled in, and that path is removed. This line's own premise — *"behind the existing per-venue build tags"* — described an architecture that has since been **retired**: `git grep "go:build"` finds no `binance`/`okx` tag anywhere in the module, and `go list -deps` with and without `-tags "binance okx"` returns byte-identical 560-package sets. Not creds-gated after all: this is public market data over a plain websocket, no API key.
