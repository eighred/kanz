...
it('never falls back to the unsafe legacy route on an older deployment', async () => {
  const get = vi.spyOn(api, 'get').mockRejectedValue(new ApiError(404, 'not found'))
  await expect(reference.price('BTC/USD')).rejects.toBeInstanceOf(ApiError)
  expect(get).toHaveBeenCalledExactlyOnceWith('/api/v1/price-observations/BTC%2FUSD')
})
...
it('encodes price identifiers and preserves every decimal digit', async () => {
  ...
  expect(get).toHaveBeenCalledWith('/api/v1/price-observations/BTC%2FUSD')
})
